...
2. **학습 Flow (Airflow + SQS + Worker)**
   ...
   - `check_data_change`: DB의 현재 데이터 건수와 콘텐츠 지문값(해시 집계)을 함께 비교해 변화가 없을 경우 파이프라인을 중지(Short-circuit)합니다.
...
배포 전 스테이징 E2E 검증: `./scripts/verify_staging_e2e.sh` (상세: [docs/rollout-guardrails.md](docs/rollout-guardrails.md))
...
- **실제 흐름**: `validate_env` -> `dispatch_infer_message` -> `verify_infer_result`
- **스케줄**: 매일 UTC `02:30` (`30 2 * * *`)
- **역할**: 배치 추론 메시지를 SQS `infer-queue`에 발행 후, 결과 파일 존재/건수 검증 태스크로 완료 확인
...
- **품질 게이트**: `quality_gate_candidate` task는 기본적으로 차단 모드(`QUALITY_GATE_REQUIRED=true`). 통과 run이 없으면 DAG를 실패로 처리해 저품질 모델 승격을 방지
- **챔피언 전략**:
  ...
  - 레지스트리: `scripts/register_model.py`가 `models/registry/champion.json` 포인터를 갱신하고, 추론은 이 포인터(`approved_run_id`, `model_key`)를 조회해 실제 모델 키를 결정
  - Champion CSV: `tmdb/{approved_run_id}/*.csv` 규칙으로 train/infer/predictions 경로 표준화. 상세는 [docs/champion-csv-strategy.md](docs/champion-csv-strategy.md) 참고
...
- 기본 차단 품질 게이트: `QUALITY_GATE_REQUIRED=true`인 경우 통과 run이 없으면 실패 처리 (필요 시 환경변수로 완화 가능)
...
3. **기본값**: 개발 환경에서만 제한적으로 fallback 사용, 운영(`APP_ENV=production`)에서는 필수 값 미설정 시 즉시 실패
