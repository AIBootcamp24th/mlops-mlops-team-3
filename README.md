...
graph LR
  ...
  subgraph Train Pipeline
    trainSchedule[Airflow 02:00] --> syncDb[sync_tmdb_to_db]
    syncDb --> checkChange[check_data_change]
    checkChange -- "Detected" --> valQual[validate_data_quality]
    valQual --> exportS3[export_db_to_s3]
    exportS3 --> trainQueue[SQS train-queue]
    trainQueue --> trainWorker[run_train_once]
    trainWorker --> slack[Slack Notify]
    trainWorker --> wandb[WandB/S3 Model]
  end

  subgraph CD & API
    api[FastAPI Service] --> reload[Hot-reloading Task]
    reload -- "Every 10m" --> champion[S3 Champion Registry]
    wandb --> champion
  end

  subgraph Infer Pipeline
    inferSchedule[Airflow 02:30] --> inferQueue[SQS infer-queue]
...
1. **학습 Flow (Airflow + SQS + Worker)**
   - **수집**: `sync_tmdb_to_db`가 TMDB에서 데이터를 가져와 RDS에 저장합니다.
   - **변동 체크**: `check_data_change`가 이전 학습 시점 대비 데이터 증가가 있는지 확인합니다. (없으면 중단)
   - **품질 검증**: `validate_data_quality`가 결측치 및 이상치를 검증합니다.
   - **내보내기**: `export_db_to_s3`가 DB 데이터를 CSV로 추출하여 S3에 업로드합니다.
   - **학습**: `run_train_once`가 S3 데이터를 읽어 학습을 수행하며, 실시간으로 Slack 알림을 전송합니다.
   - **등록**: `quality_gate_candidate`가 성능 검증 후 최적 모델을 Champion으로 등록합니다.

2. **API 및 배포 Flow (CD)**
   - **Hot-reloading**: API 서비스는 10분마다 S3의 `champion.json`을 확인하여 새로운 모델이 등록되었을 경우 서버 재시작 없이 즉시 로드합니다.
   - **로깅**: 모든 요청 및 예측 결과는 RDS/Aurora MySQL에 비차단 방식으로 저장됩니다.
...
- **실적 흐름**: `validate_env` -> `sync_tmdb_to_db` -> **`check_data_change`** -> **`validate_data_quality`** -> **`export_db_to_s3`** -> `dispatch_train_message` -> `run_train_once` -> `quality_gate_candidate`
- **스케줄**: 매일 UTC `02:00` (`0 2 * * *`)
- **실행 기간**: `2026-02-27` ~ `2026-03-15`
- **특징**: `check_data_change` 단계에서 데이터 변화가 없으면 이후 단계를 모두 Skip(Short-circuit)하여 리소스를 절약합니다.
...
- **Hot-reloading (CD)**: 백그라운드 태스크가 주기적으로 신규 모델을 감지하여 실시간으로 반영합니다.
